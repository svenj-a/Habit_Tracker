...
class TestHabit:
    ...
    def test_create_habit(self, db_mock_data):
        habit = Habit(name="diary", desc="Write a diary entry every evening.", period="daily", goal=14
                      ).create_habit(db_mock_data)
        assert habit.name == "diary"
        assert db_mock_data.cur.execute("""SELECT name FROM habits WHERE name=?""", (habit.name,)).fetchall()
        assert not db_mock_data.cur.execute("""SELECT name FROM habits WHERE name=?""", ("blob",)).fetchall()

    def test_fetch_habit_data(self, db_mock_data):
        habit = Habit(name="sleep")
        assert habit.goal == 100
        habit = Habit(name="sleep").fetch_habit_data(db_mock_data)
        assert habit.goal == 30

    def test_complete_daily_habit(self, db_mock_data):
        habit = Habit("sleep").fetch_habit_data(db_mock_data)
        assert habit.completed_total == 30
        habit.complete_habit(db_mock_data)
        assert habit.completed_total == 31

    def test_complete_weekly_habit(self, db_mock_data):
        habit = Habit('clean bathroom').fetch_habit_data(db_mock_data)
        assert habit.completed_total == 4
        habit.complete_habit(db_mock_data)
        assert habit.completed_total == 5

    def test_complete_monthly_habit(self, db_mock_data):
        habit = Habit('call mom').fetch_habit_data(db_mock_data)
        assert habit.completed_total == 2
        habit.complete_habit(db_mock_data)
        assert habit.completed_total == 3

    def test_first_time_completion(self, db_mock_data):
        new_habit = Habit(name="read", desc="Read a few pages every evening.", period="daily", goal=14
                          ).create_habit(db_mock_data)
        new_habit.complete_habit(db_mock_data)
        assert (new_habit.completed_total, new_habit.current_streak, new_habit.longest_streak) == 1

    def test_calculate_daily_timedelta(self, db_mock_data):
        habit = Habit("sleep").fetch_habit_data(db_mock_data)
        timedelta, timespan = habit._calculate_timedelta(completed=datetime.strptime("2023-05-11 06:22:06.987654",
                                                                                     "%Y-%m-%d %H:%M:%S.%f"),
                                                         last=datetime.strptime("2023-05-10 06:22:06.987654",
                                                                                "%Y-%m-%d %H:%M:%S.%f"))
        assert timedelta == 1, timespan == "daily"
